...
document.addEventListener("load", soncoInit, true);
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
function soncoInit() {
  document.removeEventListener("load", soncoInit, true);
  var gB = Services.prefs.getBranch("extensions.addonmanagerfix."), prefList = gB.getChildList("", {});
  var list = gB.getCharPref('organize').split(',');
  setTimeout(function() {
    for (var i in prefList) {
      var xxx = gB.getPrefType(prefList[i]);
      if (xxx == 128) {// 128 = boolean, 64 = integer, 32 = string
        try {
          document.getElementById("sonco-" + prefList[i]).setAttribute("checked", gB.getBoolPref(prefList[i]));
        } catch(ex) {}
    } }
    for (var i = 0; i < list.length; i++) {
      var aId = document.getElementById(list[i]);
      document.getElementById("categories").appendChild(aId);
    }
  }, 250);
}

function soncoTogglePref(x) {
  var hs = Services.prefs.getBranch("extensions.addonmanagerfix.").getBoolPref(x) != false ? false : true;
  Services.prefs.getBranch("extensions.addonmanagerfix.").setBoolPref(x, hs);
}

function soncoEnabledCount(x) {
  var view = document.getElementById(x), list_enabled = 0;
  for (var i = 0; i < view.childNodes.length; i++) if (view.childNodes[i].getAttribute("active") == "true") list_enabled++;
  return list_enabled + " Enabled";
}

function soncoDisabledCount(x) {
  var view = document.getElementById(x), list_disabled = 0;
  for (var i = 0; i < view.childNodes.length; i++)
    if (view.childNodes[i].getAttribute("active") == "false") list_disabled++;
  return list_disabled + " Disabled";
}

function soncoDisableItems() {
  document.getElementById("item-enabled").value = 0 + " Enabled";
  document.getElementById("item-disabled").value = 0 + " Disabled";
}

function soncoEnableItems() {
  document.getElementById("item-enabled").value =  soncoEnabledCount("addon-list");
  document.getElementById("item-disabled").value = soncoDisabledCount("addon-list");
}

function soncoSearchCount() {
  var sFilter = document.getElementById('search-filter-radiogroup'), sList = document.getElementById('search-list'), list_disabled = 0;
  for (var i = 0; i < sList.childNodes.length; i++) if (sList.childNodes[i].getAttribute('active') == 'false') list_disabled++;
  if (sFilter.value == 'local' && list_disabled == 0) document.getElementById('search-disabled').setAttribute('style', 'opacity: 0; visibility: collapse;');
  if (sFilter.value == 'local' && list_disabled != 0) document.getElementById('search-disabled').setAttribute('style', 'opacity: 1; visibility: visible;');
  if (sFilter.value == 'remote') document.getElementById('search-disabled').setAttribute('style', 'opacity: 0; visibility: collapse;');
  return list_disabled + ' Disabled';
}

function soncoLocalCount() {
  var sList = document.getElementById('search-list'), listLocal = 0;
  for (var i = 0; i < sList.childNodes.length; i++) if (sList.childNodes[i].getAttribute('remote') == 'false') listLocal++;
  return listLocal;
}

function soncoRemoteCount() {
  var sList = document.getElementById('search-list'), listRemote = 0;
  for (var i = 0; i < sList.childNodes.length; i++) if (sList.childNodes[i].getAttribute('remote') == 'true') listRemote++;
  return listRemote;
}
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
function sortElements(aElements, aSortBy, aAscending) {
  ...
  const UISTATE_ORDER = ["enabled", "pendingDisable", "pendingUninstall", "disabled"];
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
  const UISTATE_ORDER2 = [];
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
  function numberCompare(a, b) {
    if(a.match("custombuttons-button") && b.match("custombuttons-button")) {
      a = a.split("-button")[1];
      b = b.split("-button")[1];
    } 
    return a - b;
...
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
  ...
  function uiStateCompare(a, b) {
    //if (aAscending) [a, b] = [b, a]; 
    //else [b, a] = [a, b];
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    if (aAscending) [b, a] = [a, b]; 
    else [a, b] = [b, a]; 
    if(aSortBy == "uiState") return (UISTATE_ORDER.indexOf(b) - UISTATE_ORDER.indexOf(a));
    else return (UISTATE_ORDER2.indexOf(b) - UISTATE_ORDER2.indexOf(a));
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
  }
  function getValue(aObj, aKey) {
    if (!aObj) return null;
    if (aObj.hasAttribute(aKey)) return aObj.getAttribute(aKey);
    var addon = aObj.mAddon || aObj.mInstall;
    if (!addon) return null;
    if (aKey == "uiState") {
      if (addon.pendingOperations == AddonManager.PENDING_DISABLE) return "pendingDisable";
      if (addon.pendingOperations == AddonManager.PENDING_UNINSTALL) return "pendingUninstall";
      if (!addon.isActive && (addon.pendingOperations != AddonManager.PENDING_ENABLE &&
      addon.pendingOperations != AddonManager.PENDING_INSTALL)) return "disabled";
      else return "enabled";
    }
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    if (aKey == "idState") {
    ...
    if (aKey == "cbState") {
      return addon.id.split("custombuttons-button")[1];
    }
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    return addon[aKey];
  }
  ...
  for (let i = 0; i < aSortBy.length; i++) {
    ...
    if (sortBy == "uiState") aSortFuncs[i] = uiStateCompare;
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    if (sortBy == "idState" || sortBy == "cbState") aSortFuncs[i] = numberCompare;
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    else if (DATE_FIELDS.indexOf(sortBy) != -1) aSortFuncs[i] = dateCompare;
    else if (NUMERIC_FIELDS.indexOf(sortBy) != -1) aSortFuncs[i] = numberCompare;
  }
  aElements.sort(function(a, b) {
    if (!aAscending) [a, b] = [b, a];
    for (let i = 0; i < aSortFuncs.length; i++) {
      var sortBy = aSortBy[i], aValue = getValue(a, sortBy), bValue = getValue(b, sortBy);
...
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    soncoEnableItems();
    //document.getElementById("list-sorters").setAttribute("style", "opacity: .3");

    var sfl = document.getElementById('search-filter-local'), sfr = document.getElementById('search-filter-remote');
    var srch = document.getElementById('header-search').value;
    var aaa = document.getElementById('local-count'), bbb = document.getElementById('remote-count');
    aaa.setAttribute('value', soncoLocalCount());
    bbb.setAttribute('value', soncoRemoteCount());
    //document.getElementById('search-disabled').setAttribute('value', soncoSearchCount());
    document.getElementById('localFilterCount').setAttribute('selected', sfl.selected);
    document.getElementById('remoteFilterCount').setAttribute('selected', sfr.selected);
    var val1 = parseInt(aaa.value), val2 = parseInt(bbb.value);
    document.getElementById('category-search').setAttribute('counter', 'Results for "' + srch + '" ' + (val1 + val2));
...
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
      soncoEnableItems();
      var gfScripts = document.querySelector("label[value='Get user scripts']");
      var ckPlugin = document.querySelector("toolbarbutton.addon-control.global-info-plugincheck");
      var cat = document.getElementById('categories'), catId = cat.selectedItem.id;
      var cnt = document.getElementById('addon-list').childElementCount;

      document.getElementById(catId).setAttribute('counter', cnt);

      if (document.getElementById("category-userstyle").selected || document.getElementById("category-custombuttons").selected) {
        document.getElementById("id-btn").hidden = false;
        document.getElementById("button-Id").hidden = false;
      } else {
        document.getElementById("id-btn").hidden = true;
        document.getElementById("button-Id").hidden = true;
      }
      if (document.getElementById("category-custombuttons").selected) {
        document.getElementById("button-container").appendChild(document.getElementById("custombutons-add-new"));
        document.getElementById("custombutton-btn").style.display = "-moz-box";
        document.getElementById("custombutons-add-new").style.display = "-moz-box";
      } else {
        document.getElementById("custombutton-btn").style.display = "none";
        document.getElementById("custombutons-add-new").style.display = "none";
      }
      if (document.getElementById("category-greasemonkey-user-script").selected) {
        document.getElementById("button-container").appendChild(gfScripts);
        gfScripts.className = "addon-control";
        gfScripts.setAttribute("style", "display: -moz-box; padding: 3px 8px 1px 8px");
        gfScripts.setAttribute("onclick", "openURL('https://greasyfork.org/scripts')");
      } else {
        gfScripts.setAttribute("style", "display: none;");
      }
      if (document.getElementById("category-userstyle").selected) {
        document.getElementById("button-container").appendChild(document.getElementById("new-userstyle"));
        document.getElementById("button-container").appendChild(document.getElementById("install-from-url"));
        document.getElementById("new-userstyle").style.display = "-moz-box";
        document.getElementById("install-from-url").style.display = "-moz-box";
        document.getElementById("userstyle-btn").style.display = "-moz-box";
      } else {
        document.getElementById("new-userstyle").style.display = "none";
        document.getElementById("install-from-url").style.display = "none";
        document.getElementById("userstyle-btn").style.display = "none";
      }
      if (document.getElementById("category-plugin").selected) ckPlugin.style.display = "-moz-box";
      else ckPlugin.style.display = "none";
      document.getElementById("category-custombuttons").tooltipText = "Custom Buttons";
      document.getElementById("custombuttons-sort-bar").style.display = "none";
      document.getElementById("greasemonkey-sort-bar").style.display = "none";
      document.getElementById("userstyle-sorting").style.display = "none";
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    });
  },
...
var gUpdatesView = {
  ...
  _showRecentUpdates: function gUpdatesView_showRecentUpdates(aRequest) {
    ...
    AddonManager.getAllAddons(function showRecentUpdates_getAllAddons(aAddonsList) {
      ...
      gViewController.notifyViewChanged();
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
      var cnt = document.getElementById('updates-list').childElementCount;
	 document.getElementById('category-recentUpdates').setAttribute('counter', cnt);
      soncoDisableItems();
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    });
  },
...
var soncoSort = {
  getSortButtons: function() {
    return document.getElementById('sonco-sort').getElementsByTagName('button');
  },
  changeSort: function(e) {
    var button = e.target, buttons = this.getSortButtons();
    ...
    button.setAttribute('checkState', button.getAttribute('checkState') == "1" ? "2" : "1");
    button.setAttribute("checked", "true");
    this.applySort();
  },
  applySort: function() {
    var list = document.getElementById('addon-list'), buttons = this.getSortButtons();
    var checkedButton = Array.filter(buttons, function(b) { return b.hasAttribute('checkState'); })[0];
    if (checkedButton == null) checkedButton = buttons[1];
...
window.addEventListener('ViewChanged', function(e) {
  if (e.target.getAttribute("type") == "userstyle" || e.target.getAttribute("type") == "custombuttons") soncoSort.applySort();
}, false);
window.addEventListener("load", function(e) {
  if (document.getElementById("list-view").getAttribute("type") == "userstyle" || document.getElementById("list-view").getAttribute("type") == "custombuttons") soncoSort.applySort();
}, false);
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
